...
import { ArrowLeft, FilePlus2, Sparkles, ChevronRight, User } from 'lucide-react'
    ...
    <div className="p-4 pb-32 max-w-2xl mx-auto">
      ...
      <div className="space-y-4">
        {students.map(alumno => {
          ...
          return (
            <Link key={alumno.slug} href={`/informes/${alumno.slug}/nuevo`} className="block active-scale group">
              <Card variant="glass" className="flex items-center gap-5 p-5 group-hover:border-indigo-500/40 transition-all shadow-lg border-slate-200/50 dark:border-white/5">
                ...
                <div className="flex items-center gap-1 group-hover:translate-x-1 transition-transform">
                  <p className="text-indigo-600 dark:text-indigo-400 text-[10px] font-black uppercase tracking-widest hidden sm:block">Nuevo Informe</p>
      ...
      <div className="mt-8 p-6 bg-indigo-600/5 rounded-3xl border border-indigo-500/10 flex items-center gap-4">
        ...
        <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed font-medium">
          <span className="text-indigo-600 dark:text-indigo-400 font-bold">Edu-Especial v4.0 Tip:</span> Una vez dentro, podés usar el chat asistente para redactar el informe en segundos basado en los registros diarios.
        </p>
      </div>
    </div>
  )
