...
import { type Alumno, type Horario, type Diagnostico } from '@/lib/mock'
...
export default function NuevoAlumnoPage() {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    const slug = nombre.toLowerCase().replace(/\s+/g, '-') + '-' + Date.now().toString().slice(-4)
  ...
  return (
    <div className="p-6 pb-32 max-w-2xl mx-auto">
      <div className="flex items-center gap-4 mb-4 pt-6">
        ...
        <div>
          <h1 className="text-2xl font-black text-slate-900 dark:text-white px-1">Nuevo Alumno</h1>
          <p className="text-amber-600 dark:text-amber-400 text-[10px] font-bold uppercase tracking-widest px-1 mt-0.5">
            💡 Esta es la única vez que vas a tener que completarlo
          </p>
        </div>
      </div>
        ...
        <section className="space-y-4">
          ...
          <div className="flex gap-3">
            <select
              ...
              className="flex-1 bg-slate-800 text-white rounded-2xl p-4 text-sm focus:ring-2 focus:ring-indigo-500 outline-none border border-slate-700/50 appearance-none shadow-inner"
            >
            ...
            <button
              ...
              className={`flex items-center gap-2 px-6 rounded-2xl border transition-all ${
                cud ? 'bg-indigo-500/20 border-indigo-500 text-indigo-400 font-bold' : 'bg-slate-800 border-slate-700 text-slate-500'
              }`}
            >
              CUD {cud ? '✓' : '✗'}
            </button>
        ...
        <section className="space-y-4">
          ...
          <div className="space-y-3">
            <textarea
              ...
              className="w-full bg-white dark:bg-slate-800 text-slate-900 dark:text-white rounded-2xl p-4 text-sm focus:ring-2 focus:ring-indigo-500 outline-none border border-slate-200 dark:border-slate-700/50 shadow-sm dark:shadow-none min-h-[80px]"
            />
            <div className="grid grid-cols-2 gap-3">
              <select
                value={etapaLectura}
